...
// ===== MAIL DIAGNOSTIC (HAPUS SETELAH SELESAI) =====
Route::get('/bw-mail-diag', function () {
    ...
    $results[] = "=== ADMIN EMAIL FROM SETTINGS ===";
    try {
        $adminEmail = App\Models\Setting::invoiceAdminEmail();
        $results[] = "Admin email: " . ($adminEmail ?: '(NULL/KOSONG - ini masalah!)');
    } catch (\Throwable $e) {
        $results[] = "ERROR: " . $e->getMessage();
    }
    $results[] = "";

    $results[] = "=== TEST KIRIM RAW EMAIL ===";
    $testTo = request()->get('email', config('mail.from.address'));
    $results[] = "Kirim ke: " . $testTo;
    try {
        Illuminate\Support\Facades\Mail::raw(
            'TEST EMAIL Bintang Wisata - ' . now()->format('Y-m-d H:i:s'),
            fn($m) => $m->to($testTo)->subject('[TEST DIAG] Email Bintang Wisata')
        );
        ...
        $prev = $e->getPrevious();
        if ($prev) $results[] = "CAUSED BY: " . $prev->getMessage();
    }
    $results[] = "";

    $results[] = "=== TEST RENDER ORDER MAILABLE ===";
    try {
        $order = App\Models\Order::latest()->first();
        if ($order) {
            $results[] = "Order: " . $order->invoice_number . " | type: " . $order->type;
            $results[] = "Customer email: " . $order->customer_email;
            $order->loadMissing('payments');
            $mail = new App\Mail\OrderInvoiceMail($order, false);
            $html = $mail->render();
            $results[] = "Render: OK (" . strlen($html) . " bytes)";
        } else {
            $results[] = "Tidak ada order.";
        }
    } catch (\Throwable $e) {
        $results[] = "ERROR render: " . $e->getMessage();
        $results[] = "File: " . basename($e->getFile()) . " line " . $e->getLine();
    }
    $results[] = "";

    $results[] = "=== KIRIM ORDER MAILABLE KE EMAIL TEST ===";
    try {
        $order = App\Models\Order::latest()->first();
        if ($order) {
            $order->loadMissing('payments');
            Illuminate\Support\Facades\Mail::to($testTo)->send(new App\Mail\OrderInvoiceMail($order, false));
            $results[] = "STATUS: >>> BERHASIL KIRIM ORDER MAILABLE! <<<";
        }
    } catch (\Throwable $e) {
        $results[] = "ERROR kirim mailable: " . $e->getMessage();
        $results[] = "File: " . basename($e->getFile()) . " line " . $e->getLine();
    }
