...
@section('page-subtitle', 'Informasi lengkap transaksi kamu')
        ...
        <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-3">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-600">
                    Dibuat: <span class="font-semibold">{{ optional($order->created_at)->format('d M Y H:i') }}</span>
                </p>
            </div>
        ...
        <div class="grid grid-cols-1 2xl:grid-cols-12 gap-5">
            ...
            <div class="2xl:col-span-7 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                <div class="flex items-center justify-between gap-3">
                    <h2 class="text-sm font-extrabold text-slate-900">Informasi Pesanan</h2>
                ...
                <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <div class="text-xs font-semibold text-slate-500">Product</div>
                    ...
                    <div class="sm:col-span-2">
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            <div class="rounded-2xl border border-slate-200 p-4 bg-white">
                                <div class="text-xs font-semibold text-slate-500">Subtotal</div>
                            ...
                            <div class="rounded-2xl border border-slate-200 p-4 bg-white">
                                <div class="text-xs font-semibold text-slate-500">Final Price</div>
                    ...
                    @if($order->type === 'tour')
                        <div>
                            <div class="text-xs font-semibold text-slate-500">Departure</div>
                            <div class="mt-1 font-semibold text-slate-900">
                                {{ $order->departure_date ? \Carbon\Carbon::parse($order->departure_date)->format('d M Y') : '-' }}
                            </div>
                        </div>
                        <div>
                            <div class="text-xs font-semibold text-slate-500">Participants</div>
                    ...
                    @if($order->type === 'rent_car')
                        <div>
                            <div class="text-xs font-semibold text-slate-500">Pickup Date</div>
                            <div class="mt-1 font-semibold text-slate-900">
                                {{ $order->pickup_date ? \Carbon\Carbon::parse($order->pickup_date)->format('d M Y') : '-' }}
                            </div>
                        </div>
                        <div>
                            <div class="text-xs font-semibold text-slate-500">Return Date</div>
                            <div class="mt-1 font-semibold text-slate-900">
            ...
            <div class="2xl:col-span-5 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                ...
                <div class="mt-4">
                    @if(($order->payments ?? collect())->count() === 0)
                        <div class="text-sm text-slate-500">Belum ada data pembayaran.</div>
                    @else
                        <div class="space-y-3">
