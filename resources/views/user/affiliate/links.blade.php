...
<div class="space-y-5">
  <div class="flex items-start justify-between gap-3">
    <div>
      ...
      <p class="mt-1 text-sm text-slate-600">Kelola link affiliate kamu, pantau klik dan konversi, dan salin link Sales / Checkout.</p>
    </div>
  ...
  <div class="bg-white p-4 rounded-2xl border border-slate-200">
    <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-3">
      <div class="md:col-span-2">
        ...
        <input name="q" value="{{ request('q') }}" placeholder="Cari nama link / code / nama produk..."
      ...
      <div>
        ...
        <select name="type"
          ...
          <option value="ship" {{ request('type')==='ship' ? 'selected' : '' }}>Sewa Kapal</option>
        </select>
      </div>
  ...
  <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
    <div class="p-4 border-b border-slate-200">
      <div class="text-sm font-extrabold text-slate-900">Daftar Links</div>
      <div class="text-xs text-slate-600 mt-1">Gunakan Sales URL untuk landing page, Checkout URL untuk direct checkout (jika frontend mendukung auto-open checkout).</div>
    </div>
    ...
    <div class="overflow-x-auto">
      <table class="min-w-full text-sm">
        <thead class="bg-slate-50">
          <tr>
            <th class="text-left px-4 py-3 font-extrabold text-slate-600 uppercase text-xs">Nama</th>
            <th class="text-left px-4 py-3 font-extrabold text-slate-600 uppercase text-xs">Produk</th>
            ...
            <tr class="align-top">
              <td class="px-4 py-3">
                ...
                <div class="mt-1 text-xs text-slate-600">
                  Created: <span class="font-semibold">{{ optional($l->created_at)->format('d M Y, H:i') }}</span>
                </div>
              </td>
  ...
  <div class="mt-2 text-xs text-slate-500" id="copy-hint-checkout-{{ $l->id }}"></div>
          ...
          @empty
            <tr>
              <td class="px-4 py-6 text-slate-600" colspan="7">Belum ada link.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
