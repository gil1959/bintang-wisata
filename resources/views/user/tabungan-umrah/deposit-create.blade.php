...
@section('page-subtitle', 'Masukkan nominal dan upload bukti pembayaran')
...
@section('content')
    ...
    <div class="card p-5 relative overflow-hidden max-w-3xl">
        ...
        <div class="flex items-start justify-between gap-3 relative">
            <div>
                <h2 class="text-sm font-extrabold text-slate-900">Form Setoran</h2>
                <p class="text-sm text-slate-500 mt-1">Pilih rekening tujuan, masukkan nominal, lalu upload bukti.</p>
            </div>
    ...
    @if($methods->count() === 0)
        <div class="rounded-2xl border border-amber-200 bg-amber-50 p-4">
            <div class="font-extrabold text-amber-700">Belum ada rekening aktif</div>
            <div class="text-sm mt-1 text-amber-700">Admin harus menambahkan rekening manual aktif di menu Pembayaran.</div>
        </div>
    @else
        <div class="space-y-3">
                ...
                <label class="block rounded-2xl border border-slate-200 bg-white p-4 cursor-pointer hover:bg-slate-50">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            {{-- NAMA BANK --}}
                            <div class="text-xs font-semibold text-slate-500">Nama Bank</div>
                            ...
                            <div class="mt-3 flex items-center justify-between gap-3">
                                <div class="min-w-0">
                                    <div class="text-xs font-semibold text-slate-500">No Rekening</div>
                            ...
                            {{-- ATAS NAMA --}}
                            <div class="mt-3">
                                <div class="text-xs font-semibold text-slate-500">Atas Nama</div>
                                <div class="mt-1 text-sm font-extrabold text-slate-900">
                                    {{ $m->account_holder }}
                                </div>
                            </div>
        ...
        <div class="text-xs text-slate-500 mt-2">
            Pilih salah satu rekening tujuan sebelum submit setoran.
        </div>
    @endif
    ...
    @enderror
            ...
            <div>
                ...
                <input type="number" name="amount" min="1000" class="input" value="{{ old('amount') }}" placeholder="Masukkan nominal" required>
            </div>

            <div>
                <label class="label">Upload Bukti Pembayaran</label>
                <input type="file" name="proof_image" class="input" accept="image/*" required>
                <div class="text-xs text-slate-500 mt-1">Format gambar, max 4MB.</div>
            </div>
...
function toastCopy() {
    ...
    window.__copyToastTimer = setTimeout(() => { t.style.opacity = '0'; }, 1200);
}
</script>
