...
@section('page-subtitle', 'Pantau progres tabungan dan riwayat setoran')
...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-5">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm hover:shadow-md transition relative overflow-hidden">
            ...
            <div class="flex items-start justify-between relative">
                <div class="min-w-0">
                    <div class="text-xs font-extrabold text-slate-500">Progres Tabungan</div>
                    <div class="mt-2 text-2xl font-extrabold text-slate-900">{{ (int) $progress }}%</div>
                    <div class="mt-1 text-xs text-slate-500">
                        Target: <span class="font-semibold">Rp {{ number_format((int)$target, 0, ',', '.') }}</span>
                    </div>
                </div>
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm hover:shadow-md transition relative overflow-hidden">
            ...
            <div class="flex items-start justify-between relative">
                <div class="min-w-0">
                    <div class="text-xs font-extrabold text-slate-500">Saldo Saat Ini (Approved)</div>
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm hover:shadow-md transition relative overflow-hidden">
            ...
            <div class="flex items-start justify-between relative">
                <div class="min-w-0">
                    <div class="text-xs font-extrabold text-slate-500">Setoran Terakhir</div>
                    ...
                    <div class="mt-2">
                        <span class="inline-flex items-center px-2.5 py-1 rounded-xl text-xs font-extrabold border {{ $badgeCls }}">
                            {{ $lastDeposit ? $badgeLabel : 'Belum ada setoran' }}
                        </span>
                    </div>
                </div>
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm hover:shadow-md transition relative overflow-hidden">
            ...
            <div class="flex items-start justify-between relative">
                <div class="min-w-0">
                    <div class="text-xs font-extrabold text-slate-500">Aksi</div>
                    <div class="mt-2 text-sm font-extrabold text-slate-900">Tambah Setoran</div>
                    <div class="mt-1 text-xs text-slate-500">Upload bukti pembayaran</div>
    ...
    <div class="card p-5">
        <div class="flex items-center justify-between gap-3">
            <h2 class="text-sm font-extrabold text-slate-900">Riwayat Setoran</h2>
        ...
        <div class="mt-4 overflow-x-auto rounded-2xl border border-slate-200 bg-white">
            <table class="min-w-full text-sm">
                <thead class="bg-slate-50 text-slate-600">
                    <tr>
                        <th class="text-left px-5 py-3 font-bold">Tanggal</th>
                        <th class="text-left px-5 py-3 font-bold">Nominal</th>
                        <th class="text-left px-5 py-3 font-bold">Status</th>
                        <th class="text-right px-5 py-3 font-bold">Aksi</th>
                    </tr>
                </thead>
