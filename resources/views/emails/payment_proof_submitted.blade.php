    ...
    <div style="padding:20px; color:#0f172a;">
      @if($isAdminCopy)
        <p style="margin:0 0 12px; padding:10px 12px; background:#fff7ed; border:1px solid #fed7aa; border-radius:12px;">
          Ini notifikasi untuk admin: user sudah upload bukti pembayaran.
        </p>
      @endif

      <p style="margin:0 0 12px;">Halo <b>{{ $isAdminCopy ? 'Admin' : $order->customer_name }}</b>,</p>
      <p style="margin:0 0 16px;">
        Bukti pembayaran untuk invoice <b>{{ $order->invoice_number }}</b> sudah berhasil diupload dan saat ini
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse: collapse;">
        ...
        <tr>
          ...
          <td style="padding:10px 0;"><b>Menunggu Verifikasi</b></td>
        </tr>
      </table>
      ...
      <div style="margin-top:18px; padding:12px 14px; background:#f1f5f9; border-radius:12px;">
        ...
        <div style="font-size:14px;">
          ...
          @else
            Admin akan memeriksa bukti transfer Anda. Jika sudah diverifikasi, Anda akan menerima email konfirmasi.
          @endif
        </div>
      </div>
