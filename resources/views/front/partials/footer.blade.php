...
<footer class="relative overflow-hidden bg-slate-950 text-slate-100">
    ...
    <div class="relative container mx-auto px-4 py-14">
        ...
        : 'Partner perjalanan terpercaya untuk menjelajahi keindahan Indonesia. Paket wisata premium dengan harga bersahabat.'
                ...
                @if(isset($footerLogos) && $footerLogos->count())
                <div class="mt-7 flex flex-wrap items-center gap-3">
                    @foreach($footerLogos as $lg)
                    @php
                    $src = $lg->image_path ? asset('storage/' . $lg->image_path) : null;
                    @endphp

                    @if($lg->url)
                    <a href="{{ $lg->url }}" target="_blank" rel="noopener"
                        class="inline-flex items-center justify-center rounded-xl border border-white/10 bg-white/5 px-3 py-2 hover:bg-white/10 transition">
                        <img src="{{ $src }}" alt="{{ $lg->name }}" class="h-8 w-auto object-contain">
                    </a>
                    @else
                    <div class="inline-flex items-center justify-center rounded-xl border border-white/10 bg-white/5 px-3 py-2">
                        <img src="{{ $src }}" alt="{{ $lg->name }}" class="h-8 w-auto object-contain">
                    </div>
                    @endif
                    @endforeach
