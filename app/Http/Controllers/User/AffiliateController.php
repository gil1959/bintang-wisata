...
if ($status === 'pending') {
    return back()->with('success', 'Pengajuan kamu sudah masuk. Tinggal tunggu admin review.');
}
    ...
    return redirect()->route('user.affiliate.apply')->with('success', 'Pengajuan affiliate berhasil dikirim.');
}
    ...
    public function storeLink(Request $request)
{
    ...
    if ($data['product_type'] === 'tour') {
        ...
        $product = $p; $slug = $p->slug; $productName = $p->title;
        ...
        $product = $p; $slug = $p->slug; $productName = $p->name;
...
'utm_term' => $data['utm_term'] ?? null,
    ...
    return redirect()->route('user.affiliate.links')->with('success', 'Affiliate link berhasil dibuat.');
}
...
public function storeUserCoupon(Request $request)
{
    ...
    return back()->with('success','Coupon berhasil ditambahkan ke akun kamu.');
}
...
if ((float) $data['amount'] > (float) $balance) {
    return back()->withErrors(['amount' => 'Saldo tidak cukup untuk nominal penarikan ini.'])->withInput();
}
...
if ($remaining > 0) {
    return back()->withErrors([
        'amount' => 'Nominal penarikan harus PAS dengan total komisi approved yang tersedia. Coba ambil nominal yang sesuai (mis. gabungan beberapa komisi yang ada).'
    ])->withInput();
}
