...
class HotelPackageController extends Controller
{
    ...
    public function update(Request $request, HotelPackage $hotel_package)
    {
        ...
        return redirect()->route('partner.hotel-packages.index')
            ->with('success', 'Paket hotel/vila berhasil diperbarui dan menunggu peninjauan ulang admin.');
    }

    public function deletePhoto($photo)
    {
        abort_unless(auth()->user()->partner_type === 'agency_hotel_vila', 403);
        $photoItem = HotelPackagePhoto::with('hotelPackage')->findOrFail($photo);
        abort_unless((int)$photoItem->hotelPackage->created_by_partner_id === (int)auth()->id(), 403);

        if (!empty($photoItem->file_path)) {
